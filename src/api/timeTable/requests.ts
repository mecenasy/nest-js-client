...
export const deleteSubjectFromTimeTable = async (data: TimeTableData) =>
  await api.delete(timeTable, { data });

export const getCalendar = async () => await api.get(calendar);

export const getTimeTableByGroup = async (group: string, year: string) =>
  await api.get(groupPath(group, year));

export const getTimeTableBySpecialty = async (specialty: string) =>
  await api.get(specialtyPath(specialty));
...
export const moveSubjectInTimeTable = async (data: MoveTimeTableData) =>
  await api.patch(timeTable, data);
